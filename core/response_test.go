...
var _ = Describe("ResponseWriter tests", func() {
	...
	// 		Expect(1).To(Equal(len(proxyResponse.Headers["Content-Type"])))
	// 		Expect("application/json").To(Equal(proxyResponse.Headers["Content-Type"][0]))
	// 	})
	...
	// 		Expect(2).To(Equal(len(proxyResponse.Headers["Set-Cookie"])))
	// 		Expect("csrftoken=foobar").To(Equal(proxyResponse.Headers["Set-Cookie"][0]))
	// 		Expect("session_id=barfoo").To(Equal(proxyResponse.Headers["Set-Cookie"][1]))
	// 	})
	// })
